...
// Import das imagens (ajuste o caminho se necessário)
...
import {
  ...
  Coins,
  EmptyFooterText,
  CharacterImage, // Adicionado no styles
} from "./styles";
...
export default function HomeScreen({ navigation }: Props) {
  ...
  return (
    <Container>
      ...
      <MainArea>
        {/* LÓGICA DA IMAGEM: Se estiver rodando mostra trono, se não, positivo */}
        <CharacterImage 
      ...
      <Footer>
        ...
        {!footerData ? (
          <EmptyFooterText>Ainda não tem sessões. Bora inaugurar o trono 👑</EmptyFooterText>
        ) : (
          <FooterCard
            onPress={() => navigation.navigate("SessionDetail", { session: footerData.session })}
          >
            <RowTop>
              <Left>
                <TimeRange>{footerData.startStr} → {footerData.endStr}</TimeRange>
      ...
      <Modal visible={menuVisible} transparent animationType="fade">
        <MenuBackdrop onPress={() => setMenuVisible(false)}>
          <MenuCard>
            <MenuItem onPress={() => { setMenuVisible(false); navigation.navigate("History"); }}>
              <MenuRow>
                <FontAwesome5 name="receipt" size={16} color="#3b2416" />
                <MenuText>Histórico</MenuText>
              </MenuRow>
            </MenuItem>
            <MenuItem onPress={() => { setMenuVisible(false); navigation.navigate("Profile"); }}>
              <MenuRow>
                <FontAwesome5 name="user" size={16} color="#3b2416" />
                <MenuText>Perfil</MenuText>
              </MenuRow>
            </MenuItem>
            <MenuItem onPress={() => { setMenuVisible(false); navigation.navigate("Setup"); }}>
              <MenuRow>
